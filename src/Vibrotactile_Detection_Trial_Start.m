...
behavior.session.trial(t).start();                                          %Start the trial.
...
behavior.session.trial(t).signal.force.write(pre_samples);                  %Write the pre-trial samples to the trial signal.
behavior.session.trial(t).time.start.micros = pre_samples(1,end);           %Recording the starting microsecond clock value.

cla(behavior.ui.ax.data);                                                   %Create the psychophysical plot axes.
hold(behavior.ui.ax.data, 'on');                                            %Hold the axes for multiple plots.
x_limits = [-0.05, 1.05]*behavior.session.trial(t).params.dur;              %Calculate x-axis bounds.           
set(behavior.ui.ax.data,'xlim',x_limits,'ylim',[-0.1,1.1]);                 %Set the x- and y-axis limits.

ipi = behavior.session.trial(t).params.vib_ipi/1000;                        %Convert the inter-pulse interval to seconds.
pulse_dur = behavior.session.trial(t).params.vib_dur/1000;                  %Convert the pulse duration to seconds.
y = [0 1 1 0 0];                                                            %Create y-coordinates for a box showing the hit window.
x = behavior.session.trial(t).params.hold_time + ...
...
for i = 1:behavior.session.trial(t).params.vib_n                            %Step through each pulse.
    ...
    if i >= behavior.session.trial(t).params.gap_start && ...
            i <= behavior.session.trial(t).params.gap_stop                  %If this is the gap pulse...
        if behavior.session.trial(t).params.catch == 1                      %If this is a catch trial...
...
str = sprintf(['%s - Starting Trial #%1.0f, '...
    ...
    char(behavior.session.trial(t).time.start.datetime,'HH:mm:ss'),...
    ...
    behavior.session.trial(t).params.catch);                                %Create a text string.
