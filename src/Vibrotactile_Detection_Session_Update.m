...
outcome_i = find(behavior.session.trial(t).outcome(1) == 'HMFCA') + 2;      %Find the row index for this outcome.
j = (behavior.session.trial(t).params.vib_rate == ...
...
if behavior.session.trial(t).outcome(1) == 'H'                              %If the trial resulted in a hit...
    behavior.session.trial(t).time.response = ...
        behavior.session.trial(t).params.time_held(1) - ...
        behavior.session.trial(t).params.hold_time;                         %Save the response time.
else                                                                        %Otherwise...
    behavior.session.trial(t).time.response = NaN;                          %Save a NaN.
end

%Print a message to the messagebox.
if behavior.session.trial(t).outcome(1) == 'A'                              %If the trial ended in an abort...
    str = sprintf(['%s - Trial #%1.0f (ABORT), '...
        ...
        behavior.session.trial(t).params.time_held);                        %Create a text string.
    ...
    str = sprintf(['%s - Trial #%1.0f (%s), '...
        ...
        char(behavior.session.trial(t).time.start.datetime,'HH:mm:ss'),...
        behavior.session.count.trial,...
        behavior.session.trial(t).outcome,...
        behavior.session.trial(t).params.vib_rate,...
        behavior.session.trial(t).params.gap_length,...
        behavior.session.trial(t).params.hold_time);                        %Create a text string.
...
new_table_row = {   sprintf('%1.0f', behavior.session.count.trial)...                         %Trial
                    char(behavior.session.trial(t).time.start.datetime,'hh:mm:ss'),...        %Time
                    sprintf('%1.2f', behavior.session.trial(t).params.hold_time),...          %Hold (s)
                    sprintf('%1.2f', behavior.session.trial(t).params.time_held),...          %Held (s)
                    sprintf('%1.1f', behavior.session.trial(t).params.actual_vib_rate),...    %Rate (Hz)
                    sprintf('%1.0f', behavior.session.trial(t).params.actual_gap_length),...  %Burst/Gap (ms)                    
                    behavior.session.trial(t).outcome,...                                     %Outcome
...
behavior.session.params.time_held(end+1,1) = ...
    behavior.session.trial(t).params.time_held;                             %Add the time held to the running list.
