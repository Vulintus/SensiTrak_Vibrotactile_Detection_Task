...
switch packet                                                               %Switch between the different data types.
    ...
    case 'LOADCELL_VAL_GM'                                                  %Loadcell value in grams...
        ...
        if isequal(behavior.run.root,'session')                             %If we're in a session...
            if behavior.session.trial(end).active                           %If a trial is ongoing...
                behavior.session.trial(t).signal.force.write(val);          %Add the new value to the trial buffer.
            end
        end
