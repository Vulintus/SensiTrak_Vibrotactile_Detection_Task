function Vibrotactile_Detection_Plot_Psychometrics(behavior)

%
% Vibrotactile_Detection_Plot_Psychometrics.m
%
%   copyright 2020, Vulintus, Inc.
%
%   VIBROTACTILE_DETECTION_PLOT_PSYCHOMETRICS plots the current and 
%   historical task performance on the performance plot axes.
%   
%   UPDATE LOG:
%   2020-01-24 - Drew Sloan - Function first implemented.
%   2025-05-21 - Drew Sloan - Updated to use the "Vulintus_Behavior_Class"
%                             workflow.
%
...
rates = unique(behavior.session.count.outcomes(1,:));                       %Grab all of the vibration rates.
gaps = unique(behavior.session.count.outcomes(2,:));                        %Grab all of the gap lengths
if numel(gaps) > 1                                                          %If there's multiple gaps.
    all_gaps = ...
        unique(vertcat(behavior.session.count.outcomes(2,:)',...
        behavior.session.params.actual_gap_length));                        %Find all gap values.
...
elseif numel(rates) > 1                                                     %If there's multiple rates but only one gap.
	all_rates = ...
        unique(vertcat(behavior.session.count.outcomes(1,:)',...
        behavior.session.params.actual_vib_rate));                          %Find all rate values.
...
scale = max([sum(behavior.session.count.outcomes(3:4,:),1),...
    sum(behavior.session.count.outcomes(5:6,:),1)]);                        %Find the parameters with the most trials.
...
if scale > 0                                                                %If there are any completed trials...
    for r = rates                                                           %Step through the vibration rates.
        for g = gaps                                                        %Step through the gaps.
            ...
            c = (behavior.session.count.outcomes(1,:) == r) & ...
                (behavior.session.count.outcomes(2,:) == g);                %Find the matching column in the trial counts.
            N = sum(behavior.session.count.outcomes(3:4,c),1);              %Grab the total number of non-catch trials.
            if N > 0                                                        %If there's any completed trials to plot.
                y = behavior.session.count.outcomes(3,c)/N;                 %Grab the hit rate.
            ...
            N = sum(behavior.session.count.outcomes(5:6,c),1);              %Grab the total number of catch trials.            
            if N > 0                                                        %If there's any completed trials to plot.
                y = behavior.session.count.outcomes(5,c)/N;                 %Grab the hit rate.
                if plot_type == 3                                           %If this is the only parameter set...
...
if max_t > 0                                                                %If the maximum hold time is greater than 0...                                                                    
    [N, ~] = histcounts(behavior.session.params.time_held,...
        0:max_t/20:max_t);                                                  %Create a histogram of hold times.
    ...
    for t = t_ticks                                                         %Step through the hold time ticks.                  
        ...
        if rem(t,1) == 0                                                    %If the value is a whole number...
            ...
            str = sprintf('%1.1f',t);                                       %Grab the string with one decimal.
        end
